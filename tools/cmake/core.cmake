...
macro(ark_load_platform_specific)
    ...
    include(platform/${ARK_COMPILER_ID}/platform.cmake OPTIONAL)
...
function(ark_apply_target_properties name)
    target_compile_definitions(${name} PRIVATE ${ARK_COMPILE_DEFINITIONS} ${LOCAL_COMPILE_DEFINITIONS})
...
function(ark_add_static_library name)
    ...
    target_link_libraries(${name} PRIVATE ${LOCAL_LIBS})
...
macro(ark_add_python_generated_unit UNIT_NAME WORKING_DIR SRC_PATH_LIST SCRIPT_PATH SCRIPT_DEPEND_PATH_LIST)
    ...
    add_custom_command(OUTPUT ${LOCAL_GENERATED_FILE_NAME}.h ${LOCAL_GENERATED_FILE_NAME}.cpp
        ...
        DEPENDS ${FILE_DEPENDS} ${LOCAL_SCRIPT_PATH} ${SCRIPT_DEPEND_PATH_LIST}
...
function(ark_find_vcpkg_package PACKAGE_NAME)
    set(options MODULE_MODE)
    set(oneValueArgs "")
    set(multiValueArgs "COMPONENTS" "LIBRARIES" "EXPORT_VARS")
    cmake_parse_arguments(ark_find_vcpkg_package "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})
    if(NOT ark_find_vcpkg_package_MODULE_MODE)
        list(APPEND LOCAL_FIND_PACKAGE_ARGS CONFIG)
    endif()
    if(ark_find_vcpkg_package_COMPONENTS)
        list(APPEND LOCAL_FIND_PACKAGE_ARGS COMPONENTS ${ark_find_vcpkg_package_COMPONENTS})
    endif()

    find_package(${PACKAGE_NAME} ${LOCAL_FIND_PACKAGE_ARGS} REQUIRED)
...
macro(ark_compile_definition_option name help_text value)
    option(${name} ${help_text})
    if(${${name}})
        if("${value}")
            ark_compile_definitions(-D${name}=${value})
        else()
            ark_compile_definitions(${name})
        endif()
    endif()
endmacro()
...
macro(ark_parse_target_arguments)
...
macro(ark_add_plugin_directory NAME DIR_NAME)
    ...
    ark_compile_definition_option(${LOCAL_OPT_NAME} "Build Plugin ${NAME}" ON)
    if(${${LOCAL_OPT_NAME}})
...
function(ark_add_resource_file)
    foreach(i ${ARGN})
        list(APPEND LOCAL_RES_LIST ${i})
    endforeach()
    ark_export(LOCAL_RES_LIST)
endfunction()

function(ark_add_resource_dir)
    set(LOCAL_RESOURCE_PATTERN "*.h" "*.hpp" "*.py" "*.xml" "*.json" "*.md" "*.yaml" "*.glsl" "*.vert" "*.frag" "*.geom" "*.cmake")
    foreach(i ${ARGN})
...
if(ARK_USE_STATIC_TRIPLET)
    ark_compile_definitions(-DARK_USE_STATIC_TRIPLET)
endif()
