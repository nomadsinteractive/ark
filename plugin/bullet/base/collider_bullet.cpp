...
sp<ColliderBullet> ColliderBullet::BUILDER_IMPL1::build(const Scope& args)
{
    ...
    _resource_loader_context->renderController()->addPreRenderRunRequest(collider, BooleanType::__or__(_resource_loader_context->disposed(), sp<Boolean>::make<BooleanByWeakRef<ColliderBullet>>(collider, 1)));
    return collider;
}
