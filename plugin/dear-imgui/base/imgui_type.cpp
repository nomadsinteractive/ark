...
Imgui::BUILDER::BUILDER(BeanFactory& factory, const document& manifest)
    : _manifest(manifest), _camera(sp<Camera>::make(Ark::instance().createCamera(enums::COORDINATE_SYSTEM_LHS, false, Ark::instance().renderController()->renderEngine()->isBackendLHS()))),
