#include "cmft/texture_uploader/radiance_cubemap_uploader.h"
...
RadianceCubemapUploader::RadianceCubemapUploader(sp<Bitmap> bitmap, sp<Size> size)
    : _bitmap(std::move(bitmap)), _size(std::move(size))
{
}

void RadianceCubemapUploader::initialize(GraphicsContext& graphicsContext, Texture::Delegate& delegate) {
    ...
    Bitmap::Util<float>::hflip(reinterpret_cast<float*>(input.m_data), n * 2, n, 4);
    ::cmft::imageToCubemap(input);

    ::cmft::Image output;
    ::cmft::imageCreate(output, n, n, 0, 1, 6, ::cmft::TextureFormat::RGBA32F);
    ::cmft::imageRadianceFilter(output, n, ::cmft::LightingModel::BlinnBrdf, false, 1, 8, 1, input, ::cmft::EdgeFixup::None, 6, nullptr);
...
{
    return sp<RadianceCubemapUploader>::make(_bitmap->build(args), _size->build(args));
}
