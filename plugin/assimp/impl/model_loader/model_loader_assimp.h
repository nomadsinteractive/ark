...
class ModelImporterAssimp final : public ModelImporter {
public:
...
private:
    ...
    Model loadModel(const aiScene* scene, MaterialBundle::Initializer& materialInitializer, const Manifest& manifest) const;
    Mesh loadMesh(const aiScene* scene, const aiMesh* mesh, MaterialBundle::Initializer& materialInitializer, uint32_t meshId, element_index_t vertexBase, NodeTable& boneMapping, const Vector<sp<Material> >& materials) const;
    ...
    Vector<sp<Material>> loadMaterials(const aiScene* scene, MaterialBundle::Initializer& materialInitializer) const;
