...
sp<Input> InputType::create(sp<IntArray> value, size_t size)
{
    return reserve(sp<InputObjectArray<int32_t>>::make(std::move(value)), size);
}
...
void InputType::reset(const sp<Input>& self, sp<Input> delegate)
{
...
void InputType::markDirty(const sp<Input>& self)
{
