#pragma once

#include <type_traits>

#include "core/util/math.h"

#include "core/impl/variable/variable_op2.h"

namespace ark {

class Operators {
public:
    template <typename T> using best_argument_t = std::conditional_t<(sizeof(T) > sizeof(void*)), const T&, const T>;

    template<typename T> class GT {
    public:
        bool operator()(T v1, T v2) const {
            return v1 > v2;
        }
    };

    template<typename T> class GE {
    public:
        bool operator()(T v1, T v2) const {
            return v1 >= v2;
        }
    };

    template<typename T> class LT {
    public:
        bool operator()(T v1, T v2) const {
            return v1 < v2;
        }
    };

    template<typename T> class LE {
    public:
        bool operator()(T v1, T v2) const {
            return v1 <= v2;
        }
    };

    template<typename T> class EQ {
    public:
        bool operator()(T v1, T v2) const {
            return v1 == v2;
        }
    };

    template<typename T> class AlmostEQ {
    public:
        AlmostEQ(const int32_t ulp)
            : _ulp(ulp) {
        }

        bool operator()(T v1, T v2) const {
            return Math::almostEqual(v1, v2, _ulp);
        }

    private:
        int32_t _ulp;
    };

    template<typename T> class NE {
    public:
        bool operator()(T v1, T v2) const {
            return v1 != v2;
        }
    };

    template<typename T> class Add {
    public:
        T operator()(best_argument_t<T> v1, best_argument_t<T> v2) const {
            return v1 + v2;
        }
    };

    template<typename T> class Sub {
    public:
        T operator()(best_argument_t<T> v1, best_argument_t<T> v2) const {
            return v1 - v2;
        }
    };

    template<typename T, typename P = T> class Mul {
    public:
        auto operator()(best_argument_t<T> v1, best_argument_t<P> v2) const -> decltype(v1 * v2) {
            return v1 * v2;
        }
    };

    template<typename T, typename P = T> class Div {
    public:
        auto operator()(best_argument_t<T> v1, best_argument_t<P> v2) const -> decltype(v1 / v2) {
            return v1 / v2;
        }
    };

    template<typename T, typename P = T> class FloorDiv {
    public:
        T operator()(T v1, P v2) const {
            return Math::floorDiv(v1, v2);
        }
    };

    template<typename T, typename P = T> class Dot {
    public:
        float operator()(best_argument_t<T> v1, best_argument_t<P> v2) const {
            return Math::dot(v1, v2);
        }
    };

    template<typename T, typename P = T> class Distance {
    public:
        float operator()(best_argument_t<T> v1, best_argument_t<P> v2) const {
            return Math::hypot(v1 - v2);
        }
    };

    template<typename T, typename P = T> class Distance2 {
    public:
        float operator()(best_argument_t<T> v1, best_argument_t<P> v2) const {
            return Math::hypot2(v1 - v2);
        }
    };

    template<typename T> class Hypot {
    public:
        float operator()(best_argument_t<T> v) const {
            return Math::hypot(v);
        }
    };

    template<typename T> class Hypot2 {
    public:
        float operator()(best_argument_t<T> v) const {
            return Math::hypot2(v);
        }
    };

    template<typename T, typename P = T> class Mod {
    public:
        auto operator()(T v1, P v2) const -> decltype(v1 / v2) {
            return Math::mod(v1, v2);
        }
    };

    template<typename T> class Floor {
    public:
        T operator()(T v) const {
            return std::floor(v);
        }
    };

    template<typename T> class Ceil {
    public:
        T operator()(T v) const {
            return std::ceil(v);
        }
    };

    template<typename T, typename UO, size_t N> class Transform {
    public:
        Transform(UO unaryOp)
            : _unary_op(std::move(unaryOp)) {
        }

        T operator()(const T& v) {
            T result;
            for(size_t i = 0; i < N; ++i)
                result[i] = _unary_op(v[i]);
            return result;
        }

    private:
        UO _unary_op;
    };

    template<typename T> class ModFloor {
    public:
        T operator()(const T& v1, const T& v2) const {
            return Math::modFloor(v1, v2);
        }
    };

    template<typename T> class ModCeil {
    public:
        T operator()(const T& v1, const T& v2) const {
            return Math::modFloor(v1, v2) + v2;
        }
    };

    template<typename T> class Round {
    public:
        T operator()(best_argument_t<T> v1) const {
            return Math::round(v1);
        }
    };

    template<typename T, typename P = T> class Pow {
    public:
        float operator()(T v1, P v2) const {
            return static_cast<float>(std::pow(v1, v2));
        }
    };

    template<typename T> class And {
    public:
        auto operator()(T v1, T v2) const -> decltype(v1 && v2) {
            return v1 && v2;
        }
    };

    template<typename T> class Or {
    public:
        auto operator()(T v1, T v2) const -> decltype(v1 || v2) {
            return v1 || v2;
        }
    };

    template<typename T> class Not {
    public:
        T operator()(T val) {
            return !val;
        }
    };

    template<typename T> class Neg {
    public:
        T operator()(best_argument_t<T> val) const {
            return -val;
        }
    };

    template<typename T> class Abs {
    public:
        T operator()(best_argument_t<T> val) const {
            return Math::abs(val);
        }
    };

    template<typename T> class Min {
    public:
        T operator()(T v1, T v2) const {
            return v1 < v2 ? v1 : v2;
        }
    };

    template<typename T> class Max {
    public:
        T operator()(T v1, T v2) const {
            return v1 > v2 ? v1 : v2;
        }
    };

    template<typename T, typename U> class Extend {
    public:
        auto operator()(const T& v1, const U& v2) const -> decltype(v1.extend(v2)) {
            return v1.extend(v2);
        }
    };

    template<typename T, typename P> class Cast {
    public:
        P operator()(T v1) const {
            return static_cast<P>(v1);
        }
    };

    class Atan2 {
    public:
        float operator()(const float v1, const float v2) const {
            return Math::atan2(v1, v2);
        }
    };

    template<typename T, typename U> class Subscript {
    public:
        Subscript(const int32_t idx)
            : _idx(idx) {
        }

        U operator()(const T& val) const {
            return static_cast<U>(val[_idx]);
        }

    private:
        int32_t _idx;
    };

    template<typename T> class Normalize {
    public:
        T operator()(best_argument_t<T> v) const {
            return Math::normalize(v);
        }
    };

    template<typename T, typename OP> class Builder final : public ark::Builder<Variable<T>> {
    public:
        typedef sp<ark::Builder<Variable<T>>> BuilderType;

        Builder(const BuilderType& a1, const BuilderType& a2)
            : _a1(a1), _a2(a2) {
        }

        sp<Variable<T>> build(const Scope& args) override {
            return sp<VariableOP2<sp<Variable<T>>, sp<Variable<T>>, OP>>::make(_a1->build(args), _a2->build(args));
        }

    private:
        BuilderType _a1;
        BuilderType _a2;
    };
};

}
