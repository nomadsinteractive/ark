#include "core/impl/numeric/numeric_with_boundary.h"
...
NumericWithBoundary::NumericWithBoundary(const sp<Numeric>& delegate, const sp<Numeric>& boundary, const sp<Runnable>& oncross)
    : _delegate(delegate), _boundary(boundary), _oncross(oncross), _is_greater(delegate->val() > boundary->val())
{
}

float NumericWithBoundary::val()
{
...
NumericWithBoundary::BUILDER::BUILDER(BeanFactory& factory, const document& manifest)
...
sp<Numeric> NumericWithBoundary::BUILDER::build(const sp<Scope>& args)
{
    return sp<Numeric>::adopt(new NumericWithBoundary(_delegate->build(args), _boundary->build(args), _oncross->build(args)));
}
