...
Application::Application(sp<ApplicationContext> applicationContext, const ApplicationManifest& applicationManifest, sp<ApplicationDelegate> applicationDelegate)
    : _application_context(std::move(applicationContext)), _application_delegate(applicationDelegate ? std::move(applicationDelegate) : sp<ApplicationDelegate>::make()), _viewport(getViewport(applicationManifest, _application_context->renderEngine()->viewportCoordinateSystem())), _surface_size(getSurfaceSize(applicationManifest)),
...
void Application::setSurfaceUpdater(const bool alive)
{
