...
{
    const sp<Vec3> vv3 = sp<Bezier2>::make(_t ? _t->build(args) : Ark::instance().appClock()->duration(), _v->build(args), _p1->build(args), _p2->build(args), _p3->build(args), _on_arrival->build(args));
