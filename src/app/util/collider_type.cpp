...
sp<Rigidbody> ColliderType::createBody(const sp<Collider>& self, Rigidbody::BodyType bodyType, sp<Shape> shape, sp<Vec3> position, sp<Vec4> rotation, sp<Boolean> discarded)
{
    ...
    if(shape->type().hash() == Shape::TYPE_NONE)
        return sp<Rigidbody>::make(Rigidbody::Impl{sp<Rigidbody::Stub>::make(sp<Ref>::make(0, nullptr, std::move(discarded)), bodyType, std::move(shape), std::move(position), std::move(rotation)), Box(self)}, true);
