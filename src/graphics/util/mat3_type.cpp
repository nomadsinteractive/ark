...
sp<Mat3> Mat3Type::create(const sp<Vec3>& t, const sp<Vec3>& b, const sp<Vec3>& n)
{
    DASSERT((t && b && n) || (!t && !b && !n));
...
sp<Mat3> Mat3Type::mul(const sp<Mat3>& lvalue, const sp<Mat3>& rvalue)
{
    return sp<VariableOP2<sp<Mat3>, sp<Mat3>, Operators::Mul<M3, M3>>>::make(lvalue, rvalue);
}

sp<Vec3> Mat3Type::mul(const sp<Mat3>& lvalue, const sp<Vec3>& rvalue)
{
    return sp<VariableOP2<sp<Mat3>, sp<Vec3>, Operators::Mul<M3, V3>>>::make(lvalue, rvalue);
}

sp<Vec3> Mat3Type::mul(const sp<Mat3>& lvalue, const V3& rvalue)
{
    return sp<VariableOP2<sp<Mat3>, V3, Operators::Mul<M3, V3>>>::make(lvalue, rvalue);
}

sp<Vec2> Mat3Type::mul(const sp<Mat3>& lvalue, const sp<Vec2>& rvalue)
{
    return sp<VariableOP2<sp<Mat3>, sp<Vec2>, Operators::Mul<M3, V2>>>::make(lvalue, rvalue);
}

sp<Vec2> Mat3Type::mul(const sp<Mat3>& lvalue, const V2& rvalue)
{
...
void Mat3Type::fix(const sp<Mat3>& self)
{
    ensureImpl(self)->fix();
}

sp<Mat3Impl> Mat3Type::ensureImpl(const sp<Mat3>& self)
{
