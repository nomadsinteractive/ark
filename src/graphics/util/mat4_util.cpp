#include "graphics/util/mat4_util.h"
...
sp<Mat4> Mat4Util::create(const sp<Vec4>& t, const sp<Vec4>& b, const sp<Vec4>& n, const sp<Vec4>& w)
{
    DASSERT((t && b && n && w) || (!t && !b && !n && !w));
    return t ? sp<Mat4>::make<Mat4Impl>(t, b, n, w) : sp<Mat4>::make<Mat4::Const>(M4::identity());
}

sp<Mat4> Mat4Util::create(const V4& t, const V4& b, const V4& n, const V4& w)
{
    return sp<Mat4Impl>::make(t, b, n, w);
}

sp<Mat4> Mat4Util::mul(const sp<Mat4>& lvalue, const sp<Mat4>& rvalue)
{
    return sp<VariableOP2<sp<Mat4>, sp<Mat4>, Operators::Mul<M4, M4>>>::make(lvalue, rvalue);
}

sp<Vec4> Mat4Util::mul(const sp<Mat4>& lvalue, const sp<Vec4>& rvalue)
{
    return sp<VariableOP2<sp<Mat4>, sp<Vec4>, Operators::Mul<M4, V4>>>::make(lvalue, rvalue);
}

sp<Vec4> Mat4Util::mul(const sp<Mat4>& lvalue, const V4& rvalue)
{
    return sp<VariableOP2<sp<Mat4>, V4, Operators::Mul<M4, V4>>>::make(lvalue, rvalue);
}

sp<Vec3> Mat4Util::mul(const sp<Mat4>& lvalue, const sp<Vec3>& rvalue)
{
    return sp<VariableOP2<sp<Mat4>, sp<Vec3>, Operators::Mul<M4, V3>>>::make(lvalue, rvalue);
}

sp<Vec3> Mat4Util::mul(const sp<Mat4>& lvalue, const V3& rvalue)
{
    return sp<VariableOP2<sp<Mat4>, V3, Operators::Mul<M4, V3>>>::make(lvalue, rvalue);
}

sp<Mat4> Mat4Util::identity()
{
    return sp<Mat4Impl>::make();
}

void Mat4Util::fix(const sp<Mat4>& self)
{
    ensureImpl(self)->fix();
}

sp<Mat4Impl> Mat4Util::ensureImpl(const sp<Mat4>& self)
{
