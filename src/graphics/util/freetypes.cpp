...
int FreeTypes::ftNewFace(const char *filename, FT_Long face_index, FT_Face *aface)
{
...
FT_F26Dot6 FreeTypes::ftF26Dot6(uint32_t integer, uint32_t fractional)
{
...
int32_t FreeTypes::ftCalculateBaseLinePosition(FT_Face face)
{
    return face->size->metrics.descender >> 6;
}

unsigned long FreeTypes::ftStreamIOFunc(FT_Stream stream, unsigned long offset, unsigned char* buffer, unsigned long count)
{
    FTReadableStream* readableStream = reinterpret_cast<FTReadableStream*>(stream->descriptor.pointer);
    readableStream->_readable->seek(offset, SEEK_SET);
    return count ? readableStream->_readable->read(buffer, count) : 0;
}

void FreeTypes::ftStreamCloseFunc(FT_Stream stream)
{
    FTReadableStream* readableStream = reinterpret_cast<FTReadableStream*>(stream->descriptor.pointer);
