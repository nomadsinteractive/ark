...
uint8_t* Bitmap::at(const uint32_t x, const uint32_t y) const
{
...
void Bitmap::draw(const int32_t x, const int32_t y, void* buf, const uint32_t width, const uint32_t height, const uint32_t stride) const
{
    if(buf == nullptr || width == 0 || height == 0)
        return;

    const uint8_t* ptr = static_cast<uint8_t*>(buf);
    const uint32_t x_offset = x < 0 ? 0 - x : 0;
    const uint32_t y_offset = y < 0 ? 0 - y : 0;
    const uint32_t x_max = std::min(x + width, _width);
    const uint32_t y_max = std::min(y + height, _height);
    uint8_t* row = at(0, y);
    const uint32_t srcComponentSize = stride / width;
    const uint32_t dstComponentSize = _row_bytes / _width;

    DCHECK(dstComponentSize >= srcComponentSize, "Src component size cannot be greater than dest component size");

    for (int32_t i = y; i < y_max; i++, row += _row_bytes)
        if(srcComponentSize == dstComponentSize)
        {
            const uint32_t copylen = (x_max - x - x_offset) * srcComponentSize;
            memcpy(row + (x + x_offset) * dstComponentSize, ptr + (i - y - y_offset) * stride, copylen);
        }
        else
            for(uint32_t j = x + x_offset; j < x_max; ++j)
            {
                memcpy(row + j * dstComponentSize, (ptr + j * srcComponentSize) + (i - y - y_offset) * stride, srcComponentSize);
...
void Bitmap::dump(const String& filename) const
{
    ...
    if(png_ptr != NULL)
    {
        ...
        if(info_ptr != NULL && !setjmp(png_jmpbuf(png_ptr)))
        {
            ...
            const int32_t colorType = pixel_size == 1 ? PNG_COLOR_TYPE_GRAY : (pixel_size == 3 ? PNG_COLOR_TYPE_RGB : PNG_COLOR_TYPE_RGB_ALPHA);
            ...
            png_byte** row_pointers = static_cast<png_byte **>(png_malloc(png_ptr, _height * sizeof(png_byte *)));
            for(y = 0; y < _height; ++y)
            {
                png_byte* row = static_cast<png_byte *>(png_malloc(png_ptr, sizeof(uint8_t) * _width * pixel_size));
                row_pointers[y] = row;
                for(x = 0; x < _width; ++x)
                {
