...
TileMap::TileMap(const sp<LayerContext>& layerContext, uint32_t width, uint32_t height, uint32_t tileWidth, uint32_t tileHeight)
    : _layer_context(layerContext), _size(sp<Size>::make(static_cast<float>(width), static_cast<float>(height))),
        ...
        if(i >= 0)
        {
            ...
            for(int32_t j = colIdStart; j < colIdEnd; j++)
                if(j >= 0)
                {
                    ...
                    if(renderObject)
                        _layer_context->draw(x + (j - colIdStart) * _tile_width - ox, y + dy, renderObject);
                }
        }
    }
}
