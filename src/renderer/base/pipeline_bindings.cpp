...
{
    const Map<enums::ShaderStageBit, ShaderPreprocessor::Stage> stages = _stub->_pipeline_descriptor->getPreprocessedStages();
    ASSERT(!stages.empty());
    ...
    RenderBackend& renderEngine = Ark::instance().renderController()->renderBackend();
