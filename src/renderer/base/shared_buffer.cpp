...
SharedBuffer::Concat::Concat(size_t objectCount, size_t vertexCount, const array<element_index_t>& indices)
    : Uploader(objectCount * indices->size()), _object_count(objectCount), _vertex_count(vertexCount), _indices(indices)
{
}

void SharedBuffer::Concat::upload(const Uploader::UploadFunc& uploader)
{
...
SharedBuffer::Degenerate::Degenerate(size_t objectCount, size_t vertexCount, const array<element_index_t>& indices)
...
void SharedBuffer::Degenerate::upload(const Uploader::UploadFunc& uploader)
{
    size_t length = _indices->length();
    size_t size = _indices->size();
    size_t offset = 0;
