...
VertexWriter::VertexWriter(const PipelineLayout::AttributeOffsets& attributes, bool doTransform, uint8_t* ptr, size_t size, size_t stride)
    : VertexWriter(attributes, doTransform, sp<WriterMemory>::make(ptr, size, stride))
{
}

VertexWriter::VertexWriter(const PipelineLayout::AttributeOffsets& attributes, bool doTransform, sp<VertexWriter::Writer> writer)
