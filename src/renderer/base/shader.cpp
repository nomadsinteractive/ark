...
public:
    ...
    virtual sp<Shader> build(const Scope& args) override {
        ...
        return sp<Shader>::make(_render_controller->createPipelineFactory(), _render_controller, sp<PipelineLayout>::make(buildingContext), std::vector<sp<Snippet>>(), camera ? camera : _default_camera, scissor ? Rect(scissor->val()) : Rect(), _pipeline_bindings_flags);
    }
...
Shader::Shader(sp<PipelineFactory> pipelineFactory, sp<RenderController> renderController, sp<PipelineLayout> pipelineLayout, std::vector<sp<Snippet>> snippets,
    ...
    : _pipeline_factory(std::move(pipelineFactory)), _render_controller(std::move(renderController)), _pipeline_layout(std::move(pipelineLayout)), _input(_pipeline_layout->input()), _camera(camera ? camera : Camera::getDefaultCamera()),
      _snippets(std::move(snippets)), _pipeline_bindings_scissor(_render_controller->renderEngine()->toRendererScissor(pipelineBindingsScissor)), _pipeline_bindings_flag(pipelineBindingsFlag)
{
...
sp<Shader> Shader::fromStringTable(const String& vertex, const String& fragment, const sp<Snippet>& snippet, const sp<ResourceLoaderContext>& resourceLoaderContext)
{
    ...
    return sp<Shader>::make(renderController->createPipelineFactory(), renderController, sp<PipelineLayout>::make(buildingContext), std::vector<sp<Snippet>>(), nullptr, Rect(), PipelineBindings::FLAG_DEFAULT_VALUE);
}
...
sp<ShaderBindings> Shader::makeBindings(ModelLoader::RenderMode mode, PipelineBindings::RenderProcedure renderProcedure) const
{
    ...
    for(const sp<Snippet>& i : _snippets)
        shaderBindings->addSnippet(i);
...
Shader::BUILDER::BUILDER(BeanFactory& factory, const document& manifest, const sp<ResourceLoaderContext>& resourceLoaderContext)
    : _factory(factory), _manifest(manifest), _resource_loader_context(resourceLoaderContext), _stages(loadStages(factory, manifest)),
      _snippets(factory.getBuilderList<Snippet>(manifest, "snippet-draw")), _snippet(factory.getBuilder<Snippet>(manifest, Constants::Attributes::SNIPPET)),
...
sp<Shader> Shader::BUILDER::build(const Scope& args)
{
    ...
    std::vector<sp<Snippet>> drawSnippets;
