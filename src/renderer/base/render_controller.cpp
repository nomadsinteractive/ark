...
sp<Texture> RenderController::createTexture2d(sp<Bitmap> bitmap, sp<Texture::Parameters> parameters, const enums::UploadStrategy us, sp<Future> future)
{
    ...
    return createTexture(std::move(size), parameters ? std::move(parameters) : sp<Texture::Parameters>::make(Texture::TYPE_2D), sp<Texture::Uploader>::make<Texture::UploaderBitmap>(std::move(bitmap)), us, std::move(future));
}
