...
RenderTarget::RenderTarget(const sp<Device>& device)
    : _device(device)
{
    ...
    _command_pool = sp<CommandPool>::make(_device, _swap_chain.queueNodeIndex);
...
const sp<Device>& RenderTarget::device() const
{
    return _device;
}

const sp<CommandPool>& RenderTarget::commandPool() const
{
    return _command_pool;
}

VkRenderPass RenderTarget::renderPass() const
{
    return _render_pass;
}

const std::vector<VkFramebuffer>& RenderTarget::frameBuffers() const
{
    return _frame_buffers;
}

std::vector<VkCommandBuffer> RenderTarget::makeCommandBuffers() const
{
    return _command_pool->makeCommandBuffers(_swap_chain.imageCount);
}

uint32_t RenderTarget::acquire()
{
...
void RenderTarget::submit(VkCommandBuffer* commandBuffer)
{
...
void RenderTarget::setupDepthStencil()
{
...
void RenderTarget::setupRenderPass()
{
