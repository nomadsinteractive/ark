...
public:
    ...
    virtual sp<Resource> createFramebuffer(const std::vector<sp<Texture>>& colorAttachments, const sp<Texture>& depthStencilAttachments) = 0;
    virtual sp<Renderer> createFramebufferRenderer(RenderController& renderController, sp<Framebuffer> framebuffer, sp<Renderer> delegate, std::vector<sp<Texture>> drawBuffers, int32_t clearMask) = 0;
