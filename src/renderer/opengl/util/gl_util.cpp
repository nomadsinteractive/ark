...
//    static sp<ShaderBindings> bindings = sp<ShaderBindings>::make(RenderModel::RENDER_MODE_TRIANGLES, renderController, shader.pipelineLayout());
    const sp<opengl::GLPipeline> glPipeline = pipeline; // shader.getPipeline(graphicsContext, bindings);
