...
VerticesNinePatchQuadsLHS::VerticesNinePatchQuadsLHS()
    : VerticesNinePatch(36)
{
}

VerticesNinePatchQuadsLHS::VerticesNinePatchQuadsLHS(const Rect& bounds, const Rect& patches, uint32_t textureWidth, uint32_t textureHeight)
    : VerticesNinePatch(36, bounds, patches, textureWidth, textureHeight)
{
}

void VerticesNinePatchQuadsLHS::write(VertexWriter& buf, const V3& size)
{
    ...
    for(uint32_t i = 0; i < 3; i++) {
        for(uint32_t j = 0; j < 3; j++) {
            ...
            buf.writeTexCoordinate(_x[j], _y[3 - i]);
            buf.next();
            buf.writePosition(V3(xData[j], yData[i + 1], 0));
            buf.writeTexCoordinate(_x[j], _y[2 - i]);
            buf.next();
            buf.writePosition(V3(xData[j + 1], yData[i], 0));
            buf.writeTexCoordinate(_x[j + 1], _y[3 - i]);
            buf.next();
            buf.writePosition(V3(xData[j + 1], yData[i + 1], 0));
            buf.writeTexCoordinate(_x[j + 1], _y[2 - i]);
        }
    }
}
