...
VerticesNinePatchTriangleStripsLHS::VerticesNinePatchTriangleStripsLHS()
    : VerticesNinePatch(16)
{
}

VerticesNinePatchTriangleStripsLHS::VerticesNinePatchTriangleStripsLHS(const Rect& bounds, const Rect& patches, uint32_t textureWidth, uint32_t textureHeight)
    : VerticesNinePatch(16, bounds, patches, textureWidth, textureHeight)
{
}

void VerticesNinePatchTriangleStripsLHS::write(VertexWriter& buf, const V3& size)
{
    ...
    for(uint32_t i = 0; i < 4; i++) {
        for(uint32_t j = 0; j < 4; j++) {
            buf.next();
            buf.writePosition(V3(xData[j], yData[3 - i], 0));
